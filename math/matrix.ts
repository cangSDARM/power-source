import { isArray, isNumber, isString } from 'utils/type-assert';
...
export class Matrix {
  ...
  static merge(ma: Array<Array<'E' | number | MatrixInput>>) {
    ...
    for (let i = 0; i < ma.length; i++) {
      ...
      for (let j = 0; j < ma[i].length; j++) {
        ...
        const tempRow =
          cell instanceof Matrix ? cell.row : calMatrixSize(cell).row;
    ...
    for (let j = 0; j < ma[0].length; j++) {
      ...
      for (let i = 0; i < ma.length; i++) {
        ...
        const tempCol =
          cell instanceof Matrix ? cell.column : calMatrixSize(cell).column;
   ...
   */
  slice(
    a: [number, number] | number[] | Point,
    b: [number, number] | number[] | Point
  ) {
    // 取出坐标
    const [ax, ay] = a,
      [bx, by] = b;
  ...
  concatRight(...args: MatrixInput[]) {
    ...
    if (errIndex >= 0) {
      throw new Error(
        `(matrix) The row number of the ${errIndex} matrix is not equal to this.`
      );
    }
  ...
  concatDown(...args: MatrixInput[]) {
    ...
    if (errIndex >= 0) {
      throw new Error(
        `(matrix) The column number of the ${errIndex} matrix is not equal to this.`
      );
    }
   ...
   */
  filterPosition(
    callback: number | ((value: number, position: [number, number]) => boolean)
  ) {
    const position: Array<[number, number]> = [];
    const pre = isNumber(callback)
      ? (val: number) => val === callback
      : callback;
...
export const RotateMatrix: Readonly<Record<Rotate, Matrix>> = {
  [Rotate.Same]: Matrix.from([
    [1, 0],
    [0, 1],
  ]),
  [Rotate.Reverse]: Matrix.from([
    [-1, 0],
    [0, -1],
  ]),
  [Rotate.Clockwise]: Matrix.from([
    [0, 1],
    [-1, 0],
  ]),
  [Rotate.AntiClockwise]: Matrix.from([
    [0, -1],
    [1, 0],
  ]),
};
